  ...
  module Values
    class Symbol < Base
      EXPRESSION_OPERATORS = %i[
        ...
        arrayElemAt arrayToObject concatArrays filter in indexOfArray isArray map objectToArray range reduce reverseArray size slice zip
        and not or
        cmp eq gt gte lt lte ne
        cond ifNull switch
        dateFromParts dateFromString dateToParts dateToString dayOfMonth dayOfWeek dayOfYear hour isoDayOfWeek isoWeek isoWeekYear millisecond minute month second toDate week year
        literal
        mergeObjects
        objectToArray
        allElementsTrue anyElementTrue setDifference setEquals setIntersection setIsSubset setUnion
        concat dateFromString dateToString indexOfBytes indexOfCP ltrim rtrim split strLenCP strcasecmp substr substrBytes substrCP toLower toString trim toUpper
        meta
